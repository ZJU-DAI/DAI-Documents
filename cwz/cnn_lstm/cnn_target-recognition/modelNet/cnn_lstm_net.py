...
import modelNet.config as cfg

slim = tf.contrib.slim

class cnnLstmNet(object):
    ...
    def build_network(self,
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                net = slim.conv2d(net, 192, 3, scope='conv_2')
                # (?,56,56,192)
                net = slim.max_pool2d(net, 2, padding='SAME', scope='pool_2')

                net = slim.conv2d(net, 128, 1, scope='conv_3')
                net = slim.conv2d(net, 512, 3, scope='conv_4')
                # (?,28,28,512)
                net = slim.max_pool2d(net, 2, padding='SAME', scope='pool_3')

                net = slim.conv2d(net, 1024, 3, scope='conv_5')
                # (?,14,14,1024)
                net = slim.max_pool2d(net, 2, padding='SAME', scope='pool_4')

                net = tf.pad(net, np.array([[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad_2')
                # (?,7,7,1024)
                net = slim.conv2d(net, 1024, 3, 2, padding='VALID', scope='conv_6')


                net = tf.transpose(net, [0, 3, 1, 2], name='trans_1')
                # shape=(?, 50176)
                ...
                net = slim.dropout(net, keep_prob=keep_prob,
                                   is_training=is_training, scope='dropout_1')

                # shape=(?, 1470),7*7*30=1470
                net = slim.fully_connected(net, num_outputs,
                                           activation_fn=None, scope='fc_3')
        return net
