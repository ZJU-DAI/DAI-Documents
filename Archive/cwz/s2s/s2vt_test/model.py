...
class Video_Caption_Generator():
    def __init__(self, args, n_vocab, infer):
        ...
        # image embedding
        self.embed_image_W = tf.Variable(tf.random_uniform([self.dim_image, self.dim_hidden], -0.1, 0.1),
                                         name="embed_image_W")
        self.embed_image_b = tf.Variable(tf.zeros([self.dim_hidden]), name="embed_image_b")

        # word embedding for output
        self.embed_word_W = tf.Variable(tf.random_uniform([self.dim_hidden, self.n_vocab], -0.1, 0.1),
                                        name="embed_word_W")
        ...
        if self.attention == 1:
            # W for attention
            self.attention_W = tf.Variable(tf.random_uniform([self.lstm1.state_size, self.lstm2.state_size], -0.1, 0.1),
                                           name="attention_W")
            # z for attention
            self.attention_z = tf.Variable(tf.random_uniform([self.batch_size, self.lstm2.state_size], -0.1, 0.1),
                                           name="attention_z")
        ...
        if infer == False:
            ...
            # output
            self.caption = tf.placeholder(tf.int32,
                                          [self.batch_size, self.n_caption_step + 1])  # int32 for embedding_look_up
            ...
            if self.attention == 1:
                ...
                for i in range(0, self.n_caption_step):
                    ...
                    with tf.variable_scope("LSTM2", reuse=True):
                        ...
                        answer_in_onehot = tf.sparse_to_dense(sparse_mat, tf.stack([self.batch_size, self.n_vocab]),
                                                              1.0, 0.0)
